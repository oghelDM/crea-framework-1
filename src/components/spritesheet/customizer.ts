...
export class SpritesheetCustomizer extends Customizer {
  constructor(root: HTMLElement) {
    ...
    super(defaultValuesSpritesheet, { width: 80, left: 10, top: 10 });
    ...
    folder3.open();
    folder3.add({ start: () => (this.component as Spritesheet).start() }, 'start');
    folder3.add({ stop: () => (this.component as Spritesheet).stop() }, 'stop');
    folder3.add({ init: () => this.component.init(this.props, this.getCssValues()) }, 'init');

    this.component = new Spritesheet(this.props as SpritesheetType, this.getCssValues());
